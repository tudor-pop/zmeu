...
class DummyIntegrationTest extends StateTest {
    ...
    void noChanges() {
        repository.save(new Resource("main",
                DummyResource.builder()
                        .content("local")
                        .build())
        );
        var localState = repository.findByName("main").get();
    ...
    void rename() {
        ...
        var byProperties = repository.findByProperties(sec);
        Assertions.assertNotNull(byProperties);
    }
